...
- 🚀 **启动和停止** — 窗口右上角一个按钮。改监听地址或端口之后，按钮变成重启
- 🔐 **浏览器登录** — Codex、Grok、Claude、Gemini、Kimi（中文站和国际站）、Devin。令牌不出现在界面上
- 👥 **多个账号** — 同一个上游可以登多个号，请求按轮询分出去
- ⚡ **改完就生效** — 客户端密钥、出站代理、路由不用先停服务。监听地址要重启才换
- 🔌 **把工具接进来** — 服务跑起来之后，首页给出 `.env`（`OPENAI_API_KEY`、`OPENAI_BASE_URL`）和 OpenAI、AI SDK、LangChain 的示例
- 🧪 **当场试一次** — 首页「测试」里有三个 curl。没启动不能复制
- 🔄 **自动更新** — 正式版在后台检查、下载、校验，你确认后再重启安装
- 💾 **自己的一份数据** — 配置和账号在这个应用自己的目录里，不接管你已经在用的 CLI 配置
