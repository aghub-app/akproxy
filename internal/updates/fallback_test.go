...
func newFallback(t *testing.T, f *fixture) *FallbackProvider {
	...
	mux.HandleFunc("/repo/releases.atom", func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Fprintf(w, `<feed xmlns="http://www.w3.org/2005/Atom"><entry><updated>%s</updated><title>%s</title></entry></feed>`,
			time.Date(2026, 9, 22, 0, 0, 0, 0, time.UTC).Format(time.RFC3339), f.tag)
	})
