...
func (s *AtomSource) Latest(ctx context.Context) (tag string, published time.Time, err error) {
	...
	for _, e := range feed.Entries {
		tagName := strings.TrimPrefix(strings.TrimSpace(e.Title), "v")
...
func (s *AtomSource) Check(ctx context.Context, req updater.CheckRequest) (*updater.Release, error) {
	...
	asset, ext := assetNames(req.Platform, req.Arch)
...
// assetNames mirrors MatchAsset's platform naming for the deterministic
// release/download URL.
func assetNames(platform, arch string) (asset, ext string) {
	assetArch := arch
	...
	return fmt.Sprintf("akproxy-%s-%s%s", platform, assetArch, ext), ext
}
